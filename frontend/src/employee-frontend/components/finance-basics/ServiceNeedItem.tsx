...
export default React.memo(function ServiceNeedItem({
  ...
  const previousCoeffient = useMemo(
    () =>
      voucherValuesList.length > 0 ? voucherValuesList[0].voucherValues.coefficient : 1,
    [voucherValuesList]
  )

  const previousCoeffientUnder3y = useMemo(
    () =>
      voucherValuesList.length > 0 ? voucherValuesList[0].voucherValues.coefficientUnder3y : 1,
    [voucherValuesList]
  )
  ...
  const createNewVoucherValue = useCallback(
    () =>
      setEditorState({
        editing: 'new',
        form: emptyForm(serviceNeedId, previousCoeffient, previousCoeffientUnder3y)
      }),
